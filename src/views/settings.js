...
export function render(root, { store, navigate }) {
  ...
  const shareFm = form([
    { name: 'showBudget', label: 'Show money (Budget page, totals and prices)', type: 'checkbox', value: !!s.showBudget, hint: 'Off by default, because this link gets shared. With it off there is no Budget page, no totals on the Overview and no prices on items or stays. Nothing is deleted; it is hidden on this device only.' },
  ...
  root.append(section('Sharing this link', el('div', { class: 'card' },
    ...
    shareFm.node,
    el('div', { class: 'btn-row', style: { marginTop: '14px' } }, el('button', { class: 'btn btn-primary', type: 'button', onClick: () => {
      const v = shareFm.values();
      store.saveSettings({ showBudget: !!v.showBudget, showAllTrips: !!v.showAllTrips });
      toast('Saved', { kind: 'ok' });
    } }, 'Save')),
    el('div', { class: 'callout warn', style: { marginTop: '12px' } }, el('span', { class: 'ico' }, '⚠️'),
      el('div', {}, el('strong', {}, 'This hides, it does not lock. '), 'The repo is public, so the plan files themselves can still be read by anybody who goes looking on GitHub. Real secrets belong in the Private vault, which never leaves your device unencrypted.')),
  )));
  ...
  const fm = form([
    ...
    { name: 'path', label: `Data file (${store.tripRecord.short || store.tripId})`, value: store.filePath(), half: true, hint: 'Each trip has its own file in the repo. Changing this only affects the trip you are looking at.' },
  ...
  root.append(section('GitHub sync', el('div', { class: 'card' }, fm.node, el('div', { class: 'btn-row', style: { marginTop: '14px' } },
    el('button', { class: 'btn btn-primary', type: 'button', onClick: async () => {
      const { path, ...shared } = fm.values();
      store.saveSettings(shared);
      store.savePaths({ file: path || store.tripRecord.file });
      const v = { ...shared, path };
  ...
  // Vault sync
  const vs = store.vaultStatus || { state: 'off', message: '' };
  const vf = form([
    { name: 'vaultSync', label: 'Sync the Private vault between devices (encrypted)', type: 'checkbox', value: !!s.vaultSync },
    { name: 'vaultPass', label: 'Vault passphrase', type: 'password', value: s.vaultPass || '', hint: 'Choose something long you can retype on the phone (a sentence works). The vault is encrypted with it on this device before upload; without it the file on GitHub is unreadable. It is never sent anywhere.' },
  ]);
  root.append(section('Private vault sync', el('div', { class: `card ${vs.state === 'error' ? 'warn' : ''}` },
    el('div', { class: 'callout warn', style: { marginBottom: '12px' } }, el('span', { class: 'ico' }, '🔐'), el('div', {}, el('strong', {}, 'The repo is public. '), 'That is why the vault is encrypted before it leaves this device (AES-256, key from your passphrase). Use a strong passphrase, enter the same one on each device, and keep it somewhere safe: if you lose it, the synced copy cannot be recovered and you retype the vault.')),
    el('p', { class: 'small muted', style: { marginBottom: '10px' } }, `Status: ${vs.message || 'off'}`),
    vf.node,
    el('div', { class: 'btn-row', style: { marginTop: '14px' } },
      el('button', { class: 'btn btn-primary', type: 'button', onClick: async () => { const v = vf.values(); if (v.vaultSync && !v.vaultPass) { toast('Set a passphrase first', { kind: 'error' }); return; } if (v.vaultSync && !store.settings.token) { toast('Add the GitHub token above first', { kind: 'error' }); return; } store.saveSettings({ vaultSync: v.vaultSync, vaultPass: v.vaultPass }); if (!v.vaultSync) { store.setVaultStatus('off', 'Vault stays on this device'); toast('Vault sync off'); return; } try { const r = await store.pullVault(); toast(r?.adopted ? 'Vault loaded from GitHub' : r?.none ? 'Vault uploaded' : 'Vault synced', { kind: 'ok' }); } catch (e) { toast(e.message, { kind: 'error', ms: 7000 }); } } }, 'Save & sync vault'),
      el('button', { class: 'btn', type: 'button', onClick: async () => { try { await store.pushVault(); toast('Vault pushed', { kind: 'ok' }); } catch (e) { toast(e.message, { kind: 'error' }); } } }, 'Push vault now'),
      el('button', { class: 'btn', type: 'button', onClick: async () => { try { const r = await store.pullVault(); toast(r?.adopted ? 'Vault updated from GitHub' : 'Already up to date', { kind: 'ok' }); } catch (e) { toast(e.message, { kind: 'error' }); } } }, 'Pull vault now'),
    ),
  )));
...
const GUIDE = [
  ...
  { title: 'How your edits are saved (plain English)', body: `<p>Every change is saved instantly in the browser on the device you are using. That is the "On device" state in the top-right pill.</p><p>If you also add a GitHub token, each change is written to that trip's file in your GitHub repo a second or so later (<code>data/trip.json</code> for Japan, <code>data/bali.json</code> for Bali). Your other device pulls that file the next time it opens the app. Each trip syncs on its own, so editing Bali never touches Japan. GitHub is acting as a tiny shared notebook between phone and computer, and every save is a commit you can look back on.</p><p>The technical version: the app is static HTML/JS on GitHub Pages; the data layer is <code>localStorage</code> plus the GitHub Contents API, with a sha check so two devices never silently overwrite each other.</p>` },
  { title: 'Set up sync between phone and computer (5 minutes, once)', body: `<ol><li>On GitHub, go to <strong>Settings → Developer settings → Personal access tokens → Fine-grained tokens → Generate new token</strong>.</li><li>Name it "Trip app". Set an expiry after the last trip (e.g. March 2027).</li><li>Repository access: <strong>Only select repositories</strong> → pick <code>Trips</code>.</li><li>Permissions → Repository permissions → <strong>Contents: Read and write</strong>. Nothing else.</li><li>Generate, copy the token, paste it into the GitHub sync box above on each device, press <strong>Save &amp; test</strong>.</li></ol><p>The token is stored only in that device's browser. If it leaks, delete it on GitHub and make a new one; it can only touch this one repo's files.</p>` },
  { title: 'What never leaves your device (and the encrypted exception)', body: `<p>The <strong>Private vault</strong> page and every "Private note" field (booking references, ticket numbers, passport) are stored only in the browser and are never written to the plan file. This matters because the repo is public.</p><p>The two switches under <strong>Sharing this link</strong> are a different thing: they hide the money and the private trips from whoever opens the link, but the underlying files are still public on GitHub. Treat them as tidying up what people see, not as a lock.</p><p>If you switch on <strong>Private vault sync</strong> above, the vault is encrypted on your device with your passphrase (AES-256-GCM, key derived with PBKDF2) and the scrambled file <code>data/vault.enc</code> is saved to the repo. Anyone can download that file; nobody can read it without the passphrase. The passphrase itself is stored only in this device's browser, like the token. Enter the same passphrase on the phone and the vault appears there too. If two devices edit the vault before syncing, the newer copy wins.</p>` },
  { title: 'What each page is for', body: `<ul><li><strong>Plan switch</strong> (top of the menu): flips every page between the alternative plans. Items, stays, budget lines and to-dos tagged to one plan only show under that plan; untagged ones belong to both. When you add something, the “Applies to” field defaults to the plan you are looking at.</li><li><strong>Overview</strong>: countdown, the shape of the trip, and anything needing attention.</li><li><strong>Plan</strong>: the day-by-day timeline. Tap a day, tap a card to edit, + to add.</li><li><strong>Go</strong>: the on-the-move view for Japan. One full-screen card per stop with the time, where it is, how to get there and a Directions button that opens Google Maps from wherever you are standing. Swipe sideways to move through the day; swipe up or tap Done to clear a card (it is the same “done” tick as the Plan page, so it syncs). It opens on today and on whatever is happening now, using the local time where you are.</li><li><strong>Flights &amp; points</strong>: confirmed tickets, the legs still to book with options side by side, and the points maths.</li><li><strong>Stays</strong>: hotel shortlist per town. Mark one as Planned or Booked and it flows into the budget.</li><li><strong>Food</strong>: local things to eat, tick them off.</li><li><strong>Budget</strong>: built automatically from the above, plus manual lines.</li><li><strong>Checklists</strong>: to-dos with dates, grouped.</li><li><strong>Map</strong>: every place, with a link to Google Maps.</li><li><strong>Decisions</strong>: open questions with a recommendation each.</li><li><strong>Private vault</strong>: sensitive details, device-only.</li></ul>` },
